...
layer make_lstm_layer(int batch, int inputs, int outputs, int steps, int batch_normalize, int adam)
{
    ...
    *(l.wo) = make_connected_layer(batch*steps, outputs, outputs, LINEAR, batch_normalize, adam);
    ...
    l.output = safe_calloc(outputs*batch*steps, sizeof(float));
    l.state = safe_calloc(outputs*batch, sizeof(float));

    l.forward = forward_lstm_layer;
    l.update = update_lstm_layer;

    l.prev_state_cpu =  safe_calloc(batch*outputs, sizeof(float));
    l.prev_cell_cpu =   safe_calloc(batch*outputs, sizeof(float));
    l.cell_cpu =        safe_calloc(batch*outputs*steps, sizeof(float));

    l.f_cpu =           safe_calloc(batch*outputs, sizeof(float));
    l.i_cpu =           safe_calloc(batch*outputs, sizeof(float));
    l.g_cpu =           safe_calloc(batch*outputs, sizeof(float));
    l.o_cpu =           safe_calloc(batch*outputs, sizeof(float));
    l.c_cpu =           safe_calloc(batch*outputs, sizeof(float));
    l.h_cpu =           safe_calloc(batch*outputs, sizeof(float));
    l.temp_cpu =        safe_calloc(batch*outputs, sizeof(float));
    l.temp2_cpu =       safe_calloc(batch*outputs, sizeof(float));
    l.temp3_cpu =       safe_calloc(batch*outputs, sizeof(float));
    l.dc_cpu =          safe_calloc(batch*outputs, sizeof(float));
    l.dh_cpu =          safe_calloc(batch*outputs, sizeof(float));
